...
export default function Header() {
    ...
    return (<header className='mb-4 flex items-center justify-between'>
        <Link to="/" className='flex items-center gap-1'>
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="#ff0000" className="w-8 h-8 -rotate-90">
                <path strokeLinecap="round" strokeLinejoin="round" d="M6 12 3.269 3.125A59.769 59.769 0 0 1 21.485 12 59.768 59.768 0 0 1 3.27 20.875L5.999 12Zm0 0h7.5" />
            </svg>
            <span className='text-primary font-bold text-xl'>airbnb</span>
        </Link>
        <div className='md:flex hidden gap-2 border border-gray-300 rounded-full py-2 px-4 shadow-md shadow-gray-300'>
            <div>Anywhere</div>
        ...
        <Link to={userDoc ? '/account' : '/login'} className='flex items-center gap-2 border border-gray-300 rounded-full py-2 px-4 shadow-md shadow-gray-300 overflow-hidden'>
            {userDoc &&
                <p className='hidden md:inline'>Hello, {userDoc.name}</p>
            }
