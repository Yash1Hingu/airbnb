...
export default function PlacePage() {
    ...
    if (isShow) {
        return (
            <div className="absolute bg-gray-900 top-0 left-0 w-full flex flex-col p-8 items-center gap-8">
                ...
                <button onClick={(ev) => showAllPhotos(ev, false)} className="flex absolute top-4 right-4 py-2 px-4 rounded-3xl gap-2 hover:text-red-600">
    ...
    return (
        <div className="bg-gray-100 -mx-4 py-4 px-8 rounded-2xl">
            <h1 className="text-2xl mb-4 font-bold">{place.title}</h1>
            <a
                href={"https://maps.google.com/?q=" + place.address}
                className="text-xl underline hover:text-blue-600"
            >{place.address}</a>
            ...
            <div className="relative mt-6">
                ...
                <button onClick={(ev) => showAllPhotos(ev, true)} className="flex gap-1 absolute bottom-4 right-4 py-2 px-4 rounded-xl text-black">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
